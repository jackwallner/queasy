...
def ensure_draft_version(client: ASCClient, app_id: str, preferred: str | None = None) -> dict:
    ...
    if editable:
        return editable
    if preferred:
        # An explicit version is a request for that exact string, not a base to
        # bump: the ASC version has to match the build's CFBundleShortVersionString
        # or the build cannot be attached. (Asking for 1.1.0 used to create 1.1.1.)
        existing = find_version_by_string(client, app_id, preferred)
        return existing if existing else create_draft_version(client, app_id, preferred)
