...
struct PaywallFunnelTests {
    ...
    @Test func countsPitchesPerSurfaceAndInTotal() throws {
        try withFreshSuite {
            ConversionDiagnostics.recordPitchView(impressionID: "queasy_after_session")
            ConversionDiagnostics.recordPitchView(impressionID: "queasy_after_session")
            ConversionDiagnostics.recordPitchView(impressionID: "queasy_history")
            let a = ConversionDiagnostics.subscriberAttributes
            #expect(a["pitch_views_total"] == "3")
            #expect(a["pitch_views_after_session"] == "2")
            #expect(a["pitch_last"] == "history")
        }
    }
    ...
    @Test func conversionFreezesTheSurfaceAndCountAtTheMomentOfSale() throws {
        try withFreshSuite {
            ...
            ConversionDiagnostics.recordPitchView(impressionID: "queasy_after_session")
            let a = ConversionDiagnostics.subscriberAttributes
            #expect(a["converted_surface"] == "history")
    ...
    @Test func attributeKeysStayInsideRevenueCatsLimit() throws {
        try withFreshSuite {
            ...
            ConversionDiagnostics.recordPitchView(impressionID: "queasy_\(long)")
