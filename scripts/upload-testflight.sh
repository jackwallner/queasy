...
#
# Usage (from nausea/):
#   ./scripts/upload-testflight.sh [path/to/Queasy.xcarchive]
#
# Default archive: ./build/Queasy.xcarchive
#
# Alternative (API key / CI): see scripts/upload-testflight-api.sh

set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
ARCHIVE="${1:-$ROOT/build/Queasy.xcarchive}"
...
if [[ ! -d "$ARCHIVE" ]]; then
  ...
  cat >&2 <<'EOF'
  cd nausea && xcodegen generate && xcodebuild -project Queasy.xcodeproj \
    -scheme Queasy -configuration Release -destination 'generic/platform=iOS' \
    -archivePath "$(pwd)/build/Queasy.xcarchive" -allowProvisioningUpdates archive
EOF
  exit 1
fi
